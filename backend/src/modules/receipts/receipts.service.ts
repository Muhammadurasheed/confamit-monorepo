...
export class ReceiptsService {
  ...
  private async analyzeReceiptAsync(
    ...
    try {
      ...
      this.receiptsGateway.emitProgress(receiptId, 20, 'analysis_started', 'Initializing AI analysis pipeline...');
      ...
      if (anchorOnHedera) {
        this.receiptsGateway.emitProgress(receiptId, 85, 'hedera_anchoring', 'Anchoring to blockchain...');
        ...
        this.receiptsGateway.emitProgress(receiptId, 92, 'hedera_anchored', 'HCS anchor created!');
      }

      // 🔗 HEDERA FILE SERVICE: Store analysis hash on HFS (always, non-blocking)
      try {
        this.receiptsGateway.emitProgress(receiptId, 95, 'hfs_storing', 'Storing analysis proof on Hedera File Service...');
        
        const hfsResult = await this.hederaService.storeAnalysisOnHFS(receiptId, analysisResult);
        
        if (hfsResult) {
          await receiptRef.update({
            hedera_hfs: hfsResult,
          });
          this.logger.log(`✅ Analysis stored on HFS for receipt: ${receiptId}`);
        }
      } catch (hfsError) {
        this.logger.warn(`⚠️ HFS storage failed (non-critical): ${hfsError.message}`);
        // Non-blocking — receipt analysis is still complete
      }

...
export class BusinessService {
  ...
  private encryptData(data: string): string {
    // Proper encryption for production
  ...
  async approveVerification(businessId: string, approvedBy: string) {
    ...
    try {
      ...
      // 🔗 HEDERA SCHEDULED TRANSACTION: Auto-decay trust score in 6 months if not re-verified
      try {
        const sixMonthsFromNow = new Date();
        sixMonthsFromNow.setMonth(sixMonthsFromNow.getMonth() + 6);
        
        await this.hederaService.scheduleAutomatedTrustDecay(
          businessId,
          initialTrustScore,
          sixMonthsFromNow,
        );
        this.logger.log(`✅ Trust decay scheduled for business: ${businessId} at ${sixMonthsFromNow.toISOString()}`);
      } catch (scheduleError) {
        this.logger.warn(`⚠️ Scheduled trust decay failed (non-critical): ${scheduleError.message}`);
      }

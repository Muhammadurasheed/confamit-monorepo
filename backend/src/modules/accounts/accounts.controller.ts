...
export class AccountsController {
  ...
  @Get('profile/:uid')
  @ApiOperation({ summary: 'Get user profile and credit balance' })
  async getProfile(@Param('uid') uid: string) {
    return this.accountsService.getUserProfile(uid);
  }

  @Post('recharge')
  @ApiOperation({ summary: 'Recharge user credits via Paystack' })
  async rechargeCredits(@Body() dto: RechargeDto) {
    return this.accountsService.rechargeCredits(dto.uid, dto.amount, dto.reference);
  }
}
